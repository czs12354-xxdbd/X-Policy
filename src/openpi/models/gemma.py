...
@jax.custom_jvp
def _exact_zero_adapter_add(parent, residual):
    """Preserve the parent primal byte-for-byte at the closed HMCA boundary."""
    return jnp.where(residual == 0, parent, parent + residual)


@_exact_zero_adapter_add.defjvp
def _exact_zero_adapter_add_jvp(primals, tangents):
    parent, residual = primals
    parent_tangent, residual_tangent = tangents
    return _exact_zero_adapter_add(parent, residual), parent_tangent + residual_tangent


def _apply_action_layer_adapter(action_hidden, layer_index, payload):
    """Apply one selected HMCA bank inside the existing scanned block path."""
    (
        token_down,
        condition_down,
        horizon_code,
    ...
    selected_mask = selected_layers == layer_index
    bank_index = jnp.argmax(selected_mask.astype(jnp.int32))

    def selected_branch(parent):
        action_horizon = output_b.shape[1]
        enabled = context_enabled & jnp.all(jnp.isfinite(parent), axis=(1, 2))
        safe_parent = jnp.where(enabled[:, None, None], parent, jnp.zeros_like(parent))
        action = safe_parent[:, -action_horizon:]
        variance = jnp.mean(jnp.square(action.astype(jnp.float32)), axis=-1, keepdims=True)
        normalized = action * jax.lax.rsqrt(variance + jnp.asarray(1.0e-6, jnp.float32))
        token_latent = normalized.astype(jnp.float32) @ token_down[bank_index]
        condition_latent = semantic_features @ condition_down[bank_index]
        joint = jnp.tanh(
            token_latent
            + condition_latent[:, None, :]
            + horizon_code[bank_index][None, :, :]
        )
        residual = jnp.einsum(
            'bhr,hrd->bhd',
            joint,
        ...
        residual = jnp.where(enabled[:, None, None], residual, jnp.zeros_like(residual))
        full = jnp.zeros_like(parent, dtype=jnp.float32)
        full = full.at[:, -action_horizon:].set(residual)
        candidate = _exact_zero_adapter_add(parent, full.astype(parent.dtype))
        return jnp.where(enabled[:, None, None], candidate, parent)

    return jax.lax.cond(jnp.any(selected_mask), selected_branch, lambda parent: parent, action_hidden)


def _apply_layerwise_memory_attention(action_hidden, layer_index, payload):
    """Let action tokens read distinct causal PSM tokens at selected layers."""
    (
        query_down,
        key_down,
        value_down,
    ...
    def selected_branch(parent):
        enabled = (
            context_enabled
            & jnp.any(memory_token_mask, axis=-1)
            & jnp.all(jnp.isfinite(parent), axis=(1, 2))
            & jnp.all(jnp.isfinite(memory_tokens), axis=(1, 2))
        )
        safe_parent = jnp.where(enabled[:, None, None], parent, jnp.zeros_like(parent))
        variance = jnp.mean(
            jnp.square(safe_parent.astype(jnp.float32)), axis=-1, keepdims=True
        )
        normalized = safe_parent * jax.lax.rsqrt(
            variance + jnp.asarray(1.0e-6, jnp.float32)
        )
        queries = normalized.astype(jnp.float32) @ query_down[bank_index]
        keys = memory_tokens @ key_down[bank_index]
        values = memory_tokens @ value_down[bank_index]
        logits = jnp.einsum(
            'btr,bsr->bts', queries, keys, preferred_element_type=jnp.float32
        ) / jnp.sqrt(jnp.asarray(queries.shape[-1], jnp.float32))
        logits = jnp.where(
            memory_token_mask[:, None, :], logits, jnp.asarray(-1.0e9, logits.dtype)
        )
        weights = jax.nn.softmax(logits, axis=-1)
        read = jnp.einsum(
            'bts,bsr->btr', weights, values, preferred_element_type=jnp.float32
        )
        residual = (read @ output[bank_index]) * lora_scale
        residual = jnp.where(enabled[:, None, None], residual, jnp.zeros_like(residual))
        candidate = _exact_zero_adapter_add(parent, residual.astype(parent.dtype))
        return jnp.where(enabled[:, None, None], candidate, parent)

    return jax.lax.cond(
        jnp.any(selected_mask), selected_branch, lambda parent: parent, action_hidden
    )
...
class Block(nn.Module):
    ...
    def __call__(
        ...
        xs = sharding.activation_sharding_constraint(xs)
        if action_layer_adapter is not None and xs[1] is not None:
            xs = list(xs)
            xs[1] = _apply_action_layer_adapter(
                xs[1], layer_index, action_layer_adapter
            )
            xs = sharding.activation_sharding_constraint(xs)
        if memory_layer_adapter is not None and xs[1] is not None:
            xs = list(xs)
            xs[1] = _apply_layerwise_memory_attention(
                xs[1], layer_index, memory_layer_adapter
            )
            xs = sharding.activation_sharding_constraint(xs)
...
class Module(nn.Module):
    ...
    def setup(self):
        ...
        block_cls = nn.remat(
            Block,
            prevent_cse=False,
            static_argnums=(8,),  # method args exclude self; 8=deterministic
        ...
        self.layers = nn.scan(
            ...
            in_axes=(
                ...
                nn.broadcast,
                nn.broadcast,
            ),  # kv_cache/layer_index scan; remaining inputs broadcast
    ...
    def __call__(
        ...
        action_layer_adapter=None,
        memory_layer_adapter=None,
        *,
