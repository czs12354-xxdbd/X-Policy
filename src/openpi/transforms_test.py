...
def test_extract_prompt_from_lerobot_v04_task_dataframe():
    tasks = pd.DataFrame(
        {'task_index': [7, 11]},
        index=['place the cup on the tray', 'open the left drawer'],
    )
    transform = _transforms.PromptFromLeRobotTask(tasks)

    data = transform({'task_index': np.int64(11)})
    assert data['prompt'] == 'open the left drawer'

    with pytest.raises(
        ValueError, match='task_index=3 not found in task mapping'
    ):
        transform({'task_index': 3})
